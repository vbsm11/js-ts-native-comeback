...
async function func() {
    await api.save()
    console.log('saved')
    let data = await api.read()  // await может также работать с синхронными методами, он просто сразу возвращает данные
...
const api2 = {
    async save() { // любая асинхронная функция(метод) возвращает промис
        // в данном случае промис зарезолвится ничем(нет return)
    },
    async read() { // в данном случае промис зарезолвится возвращаемым объектом
        return {name: 'Vladimr'}
    }
}

const promise2 = api2.save()
